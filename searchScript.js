    ...
    $.getJSON('resorts.json', function (data) {
        $('#search').on('click', function () {
            ...
            for (var i in data.resorts) {
                ...
                if ((userDestination === data.resorts[i].destination) || (userDestination === 'Any')) {			//if checked value matches destination...
                    if ((actToMatch === 0 && comfortToMatch === 0) && (val >= data.resorts[i].price) && (result >= data.resorts[i].startDate) && (result <= data.resorts[i].endDate)) {  	/* if no activities is chosen or comfort level this line checks if chose price is higher than resorts price, than it checks if date matches betwween start and end date in resorts file... */
                        output += '<li data-id = ' + data.resorts[i].id + '>' + '<div class="resort">' + '<img src="' + data.resorts[i].picture + '"></img>' + '<div class="resort_name"> ' + '<h3>' + data.resorts[i].name + '</h3>' + '</div> ' + '<div class="resort_description">' + data.resorts[i].short_description + '</div> ' + '<div class="link">' + ' <a class="button" href=\'' + data.resorts[i].id + '.html\'> Visit page </a>' + '</div>' + '<div class="price">' + '&#163;' + data.resorts[i].price + '</div>' + '</div>' + '</li> '; /* ...then this results are displayed, all values are retrieved from JSON file, stored under output variable, li data-id has been created to use with draggable and droppable, img to display the image, resort name, short description, resorts id combined with a href used to open resorts website, and finally price used in green ribbon */
                    }  
                    else				/* if activities or comfort level has been chosen loop enters this else statement*/
                    {
                        ...
                        if ((actToMatch === 0 && comfortToMatch === 0) && (val >= data.resorts[i].price) && (result >= data.resorts[i].startDate) && (result <= data.resorts[i].endDate)) {				/* than this results are displayed */
                            output += '<li data-id = ' + data.resorts[i].id + '>' + '<div class="resort">' + '<img src="' + data.resorts[i].picture + '"></img>' + '<div class="resort_name"> ' + '<h3>' + data.resorts[i].name + '</h3>' + '</div> ' + '<div class="resort_description">' + data.resorts[i].short_description + '</div> ' + '<div class="link">' + ' <a class="button" href=\'' + data.resorts[i].id + '.html\'> Visit page </a>' + '</div>' + '<div class="price">' + '&#163;' + data.resorts[i].price + '</div>' + '</div>' + '</li> ';
                        }
                    }
                }
            ...
            output += '</ul>';
            if (result == '') alert('Please choose a Date');			/* if no date is picked this message will be displayed */
            if (output == '<ul></ul>') output = "No Matches Found!";		/*if no mathes found this msg will be displayed */
